...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $reg_no = $_POST['reg-number'];
        $password = $_POST['password'];

        if (empty($reg_no) || empty($password)) {
            die("Registration number and password are required.");
        }

        $stmt = $conn->prepare("SELECT password_hash,id FROM supervisors WHERE reg_number = ?");
